        ...
        <div class="card">
            ...
            {{-- @can('member_create')
                <div class="dt-action-buttons text-end">
                    <a class="btn btn-secondary create-new btn-primary pt-2 waves-effect waves-light" style="margin-right:12px"
                       tabindex="0" aria-controls="DataTables_Table_0" href="{{ url('admin/members/create') }}">
                        <span><i class="mdi mdi-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Add Customer</span>
                        </span>
                    </a>
                </div>
            @endcan --}}
            <div class="card-body">
                ...
                <div class="text-nowrap">
                    <table class="table table-responsive table-bordered customer_table table-striped">
                        <thead>
                            <tr>
                                <th class="text-center">SR</th>
                                <th class="text-center sortable cursor-pointer" data-sort="name">Name<span
                                        class="sort-indicator"></span></th>
                                <th class="text-center sortable cursor-pointer" data-sort="phone">phone<span
                                        class="sort-indicator"></span></th>
                                <th class="text-center sortable cursor-pointer" data-sort="phone">City<span
                                        class="sort-indicator"></span></th>
                                <th class="text-center sortable cursor-pointer" data-sort="age">Age<span
                                        class="sort-indicator"></span></th>
                                <th class="text-center sortable cursor-pointer" data-sort="email">Email<span
                                        class="sort-indicator"></span></th>
                                <th class="text-center sortable cursor-pointer" data-sort="income">Income<span
                                        class="sort-indicator"></span></th>
                                <th class="text-center sortable cursor-pointer" data-sort="service">Service<span
                                        class="sort-indicator"></span></th>
                                <th class="text-center sortable cursor-pointer" data-sort="service">Loan Status<span
                                        class="sort-indicator"></span></th>
                                <th class="text-center sortable cursor-pointer" data-sort="service">Action<span
                                        class="sort-indicator"></span></th>
                    ...
                    success: function(response) {
                        ...
                        if (data === null || data.length === 0) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                let edit_url = "{{ url('admin/customers/edit') }}";
                                let delete_url = "{{ url('admin/customers/delete') }}";
                                let customer_detail = "{{ url('admin/customers/view') }}";
                                ...
                                html += '<td class="text-center">' + data[i].tel + '</td>';
                                html += '<td class="text-center">' + data[i].cities.city + '</td>';
                                html += '<td class="text-center">' + data[i].age + '</td>';
                                html += '<td class="text-center">' + data[i].email + '</td>';
                                html += '<td class="text-center">' + data[i].income + '</td>';
                                html += '<td class="text-center">' + data[i].service_name + '</td>';
                                var buttonHtml;
                                if (data[i].loan_status == 1) {
                                    buttonHtml = '<button class="btn btn-outline-warning">Pending</button>';
                                } else if (data[i].loan_status == 2) {
                                    buttonHtml = '<button class="btn btn-outline-primary">Document Review</button>';
                                } else if (data[i].loan_status == 3) {
                                    buttonHtml = '<button class="btn btn-outline-info">Submit Agent</button>';
                                } else if (data[i].loan_status == 4) {
                                    buttonHtml = '<button class="btn btn-outline-danger">Reject</button>';
                                } else if (data[i].loan_status == 5) {
                                    buttonHtml = '<button class="btn btn-outline-success">Complete</button>';
                                } else {
                                    buttonHtml = '<button class="btn btn-outline-secondary">Unknown</button>';
                                }
                                html += '<td class="text-center">' + buttonHtml + '</td>';
                                html +=
                                    '<td class="text-center"><div class="dropdown"> <button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown" aria-expanded="false"> <i class="mdi mdi-dots-vertical"></i> </button><div class="dropdown-menu" style=""><a href="' +
                                    edit_url + '/' + data[i].id +
                                    '" id="editbutton" class="dropdown-item waves-effect"><i class="mdi mdi-pencil-outline me-1"></i> Edit</a><a href="' +
                                    customer_detail + '/' + data[i].id +
                                    '" id="viewbutton" class="dropdown-item waves-effect"><i class="mdi mdi-eye me-1"></i> View</a><a class="dropdown-item waves-effect" onclick="deleteRecord(`' +
                            delete_url + '/' + data[i].id +
                            '`,`.customer_table`)" href="javascript:void(0);"><i class="mdi mdi-trash-can-outline me-1"></i> Delete</a></div></div></td>';
                                html += '</tr>';
